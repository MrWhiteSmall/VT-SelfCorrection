...
def get_part_image(parse, parse_id, original_image):
    # 将 parse 转为 numpy 数组
    parse_array = np.array(parse)
    
    # 创建一个空白的掩码，大小与原始 parse 图像相同
    upper_mask = np.zeros_like(parse_array, dtype=np.float32)

    # 遍历 upper_id 列表，提取对应部分
    for label in parse_id:
        upper_mask += (parse_array == label).astype(np.float32)

    # 将掩码应用到原始图像上（例如 img 是原始图像）
    upper_mask_img = Image.fromarray(np.uint8(upper_mask * 255), mode='L')  # 转为二值图像（灰度）
    
    # 提取原图中 upper_mask 区域
    upper_part_image = Image.composite(original_image, Image.new("RGB", original_image.size), upper_mask_img)

    return upper_part_image
...
def get_mask_location_all(img,
                          model_type, category, 
                          model_parse: Image.Image, 
                          keypoint: dict, width=384,height=512):
    
    im_parse = model_parse.resize((width, height), Image.NEAREST)
    parse_array = np.array(im_parse)

    ay = np.array(parse_array)
    
    parse_head = None
    for id in fixed_head:
        if parse_head is None:
            parse_head = (ay == id).astype(np.float32) 
        else:
            parse_head += (ay == id).astype(np.float32)
    parse_lower = None
    for id in fixed_lower:
        if parse_lower is None:
            parse_lower = (ay == id).astype(np.float32) 
        else:
            parse_lower += (ay == id).astype(np.float32)
    parse_lower = ((ay == 9).astype(np.float32) +
                    (ay == 12).astype(np.float32) +
                    (ay == 16).astype(np.float32) +
                    (ay == 17).astype(np.float32) +
                    (ay == 18).astype(np.float32) +
                    (ay == 19).astype(np.float32))
